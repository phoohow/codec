...
bool DX12Encoder::EncodeFrame(void* pData, CodecPacket& packet)
{
    ...
    try
    {
        ...
        // Copy the provided texture into the encoder's internal input texture
        // so the encoder can encode it. We obtain the next input frame resource
        // from the NvEncoder and perform a GPU copy using a temporary command
        // list/queue and a fence to synchronize.
